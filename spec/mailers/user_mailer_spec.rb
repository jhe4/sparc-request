...
RSpec.describe UserMailer do
  ...
  context "added an authorized user" do
    before :each do
      ...
      @mail = UserMailer.authorized_user_changed_dashboard(@identity, study, @modified_identity, 'add')
    end
  ...
  context "deleted an authorized user" do
    before :each do
      ...
      @identity                 = create(:identity)
      @protocol_role            = create(:project_role, protocol: study, identity: @modified_identity, project_rights: 'approve', role: 'consultant')
      @mail = UserMailer.authorized_user_changed_dashboard(@identity, study, @modified_identity, 'destroy')
    end
