module EmailHelpers
  ...
  def assert_email_user_information(mail_response)
    # Should not display 'Epic Access' column
    ...
    expect(mail_response).not_to have_xpath "//th[text()='User Name']/following-sibling::th[text()='Contact Information']/following-sibling::th[text()='Role']/following-sibling::th[text()='Epic Access']"
  ...
  def assert_arm_information_table(mail_response)
    expect(mail_response).to have_xpath("//table//strong[text()='Protocol Arm Information']")
    service_request.arms.each do |arm|
      expect(mail_response).to have_xpath("//td[text()='#{arm.name}']/following-sibling::td[text()='#{arm.subject_count}']/following-sibling::td[text()='#{arm.visit_count}']")
    end
  end
  ...
  def assert_notification_email_tables_for_service_provider
    ...
    assert_arm_information_table(mail)
  end
  ...
  def assert_notification_email_tables_for_admin
    ...
    assert_arm_information_table(mail.body.parts.first.body)
  end
